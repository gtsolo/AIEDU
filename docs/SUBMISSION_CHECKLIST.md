...
> 🤖 **交给 Agent 自动执行？** 已准备 Trae Work 模式完整材料包（位于 `docs/work/` 目录）：
> - **spec.md** — 规格说明（目标、验收标准、材料索引、任务边界）
> - **tasks.md** — 可执行任务拆解（T1 推仓库 / T2 部署 Demo / T3 回填链接 / T4 视频 / T5 整理数据 / T6 正式提交）
> - **check_list.md** — 验收检查清单（30+ 项逐项打勾）
> - **submission_payload.json** — 提交系统结构化字段（中英文案已预置，仅需补链接）
> - **video_script.md** — 演示视频分镜脚本（6 幕 4 分钟台词+操作指引）
>
> 操作方式：Trae 切换 Work 模式 → 以上述 3 个核心 md 作为 Spec/Tasks/CheckList 启动。
